...
if not term.isColor or not term.isColor() then
        error('LagOS Requires an Advanced (Gold) Computer')
end

local mainTitle = 'LagOS Installer'
local monitor = nil
local downloads = {}
...
Settings = {
        ...
        GitHubUsername = 'Jamjardavies', --Your GitHub username as it appears in the URL
        GitHubRepoName = 'LagOS', --The repo name as it appears in the URL
...
function main()
	monitor = periph.findPeripheral("monitor")
	
	loadJSON()
	
	if http then
        Draw('HTTP enabled, attempting update...')
	else
        Draw('HTTP is required to update.')
        error('')
	end

	Draw('Determining Latest Version')
	...
	Draw('Optaining Latest Version URL')
...
function downloadBlob(v, latestReleaseTag)
	...
	if v.type == 'tree' then
		Draw('Making folder: ' .. '/' .. Settings.InstallPath .. v.path)
		...
		Draw('('..math.floor(100 * (Settings.DownloadedBytes / Settings.TotalBytes)) .. '%) Downloaded: ' .. Settings.InstallPath .. v.path)
